...
**code sea → glyph building → galaxy** with scroll; the galaxy persists as the
background below the hero. The building is drawn *entirely from glyphs* — the
photo (`public/building/bina.webp`) is only an offscreen sampling source and
is never displayed.
